...
public class RoboAereo extends Robo {
    ...
    public void subir(int deltaY) {
        if (deltaY >= 0)  // Verifica se o valor de deltaY não é negativo
            setAltitude(getAltitude() + deltaY);  // Aumenta a altitude do robô
        else
            System.out.println("O número não pode ser negativo!");  // Exibe uma mensagem de erro caso deltaY seja negativo
    }

    // Método para o robô aéreo descer, diminuindo sua altitude
    public void descer(int deltaY) {
        if (deltaY >= 0)  // Verifica se o valor de deltaY não é negativo
            setAltitude(getAltitude() - deltaY);  // Diminui a altitude do robô
        else
            System.out.println("O número não pode ser negativo!");  // Exibe uma mensagem de erro caso deltaY seja negativo
    }
}
